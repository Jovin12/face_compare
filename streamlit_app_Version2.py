...
def extract_face_from_image(image_path):
    """Extract face from image using crop_license_face function"""
    try:
        # Use the existing crop_license_face function
        # Create a temporary path for the cropped face
        temp_path = tempfile.NamedTemporaryFile(delete=False, suffix='.jpg')
        temp_path.close()
        
        cropped_path = crop_license_face(image_path, temp_path.name)
...
def compare_faces(face1_path, face2_path):
    """Compare two face images using compare_id_photo function"""
    try:
        # The compare_id_photo function expects file paths
        # It returns True/False, so we need to get more details
...
if st.session_state.license_path and not st.session_state.cropped_license_path:
    with st.spinner("Extracting face from driver's license..."):
        # Use the crop_license_face function
...
with st.sidebar:
    ...
    # Optional: Add a note about the CLI tool
    st.markdown("---")
    st.markdown("### 🛠️ Alternative CLI Tool")
    st.markdown("""
    You can also use the command-line tools:
    - `take_img.py` - Capture license photo
    - `crop_license.py` - Extract face from license
    - `compare_faces.py` - Compare two face images
    """)
